...
@SpringBootTest(properties = {
        "download-cache.dir=build/test-data/com.moneysupermarket.componentcatalog.service.scanners.gradle.GradleScannerErrorTest/download-cache", 
        "url-exists-cache.dir=build/test-data/com.moneysupermarket.componentcatalog.service.scanners.gradle.GradleScannerErrorTest/url-exists-cache", 
        "gradle.pom-cache-dir=build/test-data/com.moneysupermarket.componentcatalog.service.scanners.gradle.GradleScannerErrorTest/gradle/pom-cache"
})
...
public class GradleScannerErrorTest extends BaseCodebaseScannerTest {
    ...
    public void shouldScanBuildscriptMissingRepositoryBuild() {
        ...
        assertThat(error.getCause().getMessage()).containsPattern("^Failed to process build file \\\"[^\\\"]+/service/src/test/resources/com/moneysupermarket/componentcatalog/service/scanners/gradle/GradleScannerErrorTest/BuildscriptMissingRepository/build.gradle\\\" for THIS_PROJECT project mode and BUILDSCRIPT_DEPENDENCIES process phase$");
    ...
    public void shouldScanBuildscriptMissingRepositoryWithEmptyPluginsBuild() {
        ...
        assertThat(error.getCause()).isNotNull();
        assertThat(error.getCause().getScannerId()).isEqualTo(SCANNER_ID);
        assertThat(error.getCause().getMessage()).containsPattern("^Failed to process build file \\\"[^\\\"]+/service/src/test/resources/com/moneysupermarket/componentcatalog/service/scanners/gradle/GradleScannerErrorTest/BuildscriptMissingRepositoryWithEmptyPlugins/build.gradle\\\" for THIS_PROJECT project mode and BUILDSCRIPT_DEPENDENCIES process phase$");
        assertThat(error.getCause().getCause()).isNotNull();
        assertThat(error.getCause().getCause().getScannerId()).isEqualTo(SCANNER_ID);
        assertThat(error.getCause().getCause().getMessage()).isEqualTo("Failed to create software item for artifact");
        assertThat(error.getCause().getCause().getCause()).isNotNull();
