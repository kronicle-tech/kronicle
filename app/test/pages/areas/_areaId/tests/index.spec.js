...
describe('Index', () => {
  ...
  async function createWrapper() {
    wrapper = await createPageWrapper(Index, {
      route,
      serviceRequests: {
        '/v1/areas/test-area-id-1?testOutcome=fail&fields=area(id,name,components(id,name,typeId,tags,teams,platformId,testResults))':
          {
            responseBody: { area },
          },
      },
  ...
  describe('when Get Area service endpoint returns an area', () => {
    ...
    test('has the right page title', async () => {
      ...
      expect(wrapper.vm.$metaInfo.title).toBe(
        'Component Catalog - Test Area Name 1 Area - Tests'
      )
    })
