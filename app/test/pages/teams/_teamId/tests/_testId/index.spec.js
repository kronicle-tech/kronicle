  ...
  async function createWrapper() {
    wrapper = await createPageWrapper(Index, {
      ...
      serviceRequests: {
        ...
        '/v1/teams/test-team-id-1?fields=team(id,name,components(id,name,typeId,tags,teams,platformId,testResults))':
          {
            responseBody: { team },
          },
      },
  ...
  describe('when Get Test service endpoint returns a test and the Get Team service endpoint returns an team', () => {
    ...
    test('has the right page title', async () => {
      ...
      expect(wrapper.vm.$metaInfo.title).toBe(
        'Component Catalog - Test Team Name 1 Team - test-test-id-1 Test'
      )
    })
