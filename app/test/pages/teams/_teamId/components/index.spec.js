...
describe('Index', () => {
  ...
  async function createWrapper() {
    wrapper = await createPageWrapper(Index, {
      route,
      serviceRequests: {
        '/v1/teams/test-team-id-1?fields=team(id,name,components(id,name,discovered,typeId,description,tags,teams,platformId))':
          {
            responseBody: { team },
          },
      },
  ...
  describe('when Get Team service endpoint returns a team', () => {
    ...
    test('has the right page title', async () => {
      ...
      expect(wrapper.vm.$metaInfo.title).toBe(
        'Kronicle - Test Team Name 1 Team - Components'
      )
    })
