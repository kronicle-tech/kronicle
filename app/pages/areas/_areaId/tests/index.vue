...
export default Vue.extend({
  ...
  async asyncData({ $config, route, store }) {
    const area = await fetch(
      `${$config.serviceBaseUrl}/v1/areas/${route.params.areaId}?testOutcome=fail&fields=area(id,name,components(id,name,typeId,tags,teams,platformId,testResults))`
    )
      .then((res) => res.json())
  ...
  head(): MetaInfo {
    return {
      title: `Component Catalog - ${this.area.name} Area - Tests`,
    }
  },
})
</script>
