<template>
  <div class="m-3">
    <h1 class="text-info my-3">{{ area.name }} - Errors</h1>
...
export default Vue.extend({
  ...
  async asyncData({ $config, route, store }) {
    const area = await fetch(
      `${$config.serviceBaseUrl}/v1/areas/${route.params.areaId}?fields=area(id,name,components(id,name,type,tags,teams,platformId,scannerErrors))`
    )
      .then((res) => res.json())
  ...
  head(): MetaInfo {
    return {
      title: `Kronicle - ${this.area.name} - Errors`,
    }
  },
})
</script>
