...
export default Vue.extend({
  ...
  async asyncData({ $config, route, store }) {
    const area = await fetch(
      `${$config.serviceBaseUrl}/v1/areas/${route.params.areaId}?stateType=doc&fields=area(id,name,components(id,name,typeId,tags,description,notes,responsibilities,teams,platformId,states))`
    )
      .then((res) => res.json())
