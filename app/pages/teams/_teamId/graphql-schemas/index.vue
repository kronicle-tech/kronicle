...
export default Vue.extend({
  ...
  async asyncData({ $config, route, store }) {
    const team = await fetch(
      `${$config.serviceBaseUrl}/v1/teams/${route.params.teamId}?stateType=graphql-schemas&fields=team(id,name,components(id,name,typeId,tags,teams,platformId,states))`
    )
      .then((res) => res.json())
