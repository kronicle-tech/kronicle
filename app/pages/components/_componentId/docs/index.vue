<template>
  <div class="m-3">
    <h1 class="text-info my-3">{{ component.name }} - Docs</h1>
...
export default Vue.extend({
  ...
  async asyncData({ $config, route, store }) {
    ...
    const component = await fetch(
      `${$config.serviceBaseUrl}/v1/components/${route.params.componentId}?stateType=doc&fields=component(id,name,teams,states)`
    )
      .then((res) => res.json())
  ...
  head(): MetaInfo {
    return {
      title: `Kronicle - ${this.component.name} - Docs`,
    }
  },
})
</script>
